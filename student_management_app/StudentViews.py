...
def student_home(request):
    ...
    semester = Semesters.objects.get(id=student_obj.semester_id.id)
    level = Levels.objects.get(id=student_obj.level_id.id)
    q1 = Q(course_id=course)
    q2 = Q(semester_id=semester)
    q3 = Q(level_id=level)

    subjects = Subjects.objects.filter(q1 & q2 & q3).count()
    subjects_data = Subjects.objects.filter(q1 & q2 & q3)
    ...
    subject_data = Subjects.objects.filter(semester_id=student_obj.semester_id)
    subject_data = Subjects.objects.filter(level_id=student_obj.level_id)
    ...
    schedule_count = Schedules_m.objects.filter(staff_id=request.user.id)
    schedule_all = Schedules_m.objects.filter(staff_id=request.user.id)
    schedule_name_list = []
    schedule = Schedules_m.objects.filter(staff_id=request.user.id)
    for schedule in schedule_all:
        ...
        subjects = Subjects.objects.get(id=schedule.subject_id.id)
...
def student_view_attendance(request):
    ...
    semester = student.semester_id  # Added to retrieve student's semester

    # Filter subjects based on course ID, level, and semester
    subjects = Subjects.objects.filter(course_id=course, level_id=level, semester_id=semester)
...
def all_subject(request):
    ...
    return render(
        request,
        "student_template/all_subject.html",
        {"subjects": subjects},
    )
...
def views_schedule(request):
    student = Students.objects.get(admin=request.user.id)
    course = student.course_id
    schedules = Schedules_m.objects.filter(course_id=course)
    return render(
        request,
        "student_template/views_schedule.html",
        {"schedules": schedules},
    )
