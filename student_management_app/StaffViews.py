...
def staff_home(request):
    ...
    for subject in subjects:
        ...
        semester_list.append(semester.semester_name)
        attendance_list.append(attendance_count1)
    students_count = Students.objects.filter(course_id=request.user.id)
    students_counts = Students.objects.filter(admin_id=request.user.id)
...
def staff_take_attendance(request):
    ...
    return render(
        ...
        {"subjects": subjects, "courses": courses, "semesters":semesters, "levels":levels,"session_years": session_years},
    )
...
def get_students(request):
    ...
    students = Students.objects.filter(
        course_id=subject.course_id,level_id=subject.level_id, semester_id=subject.semester_id
    )
    list_data = []
...
def manage_subjectss(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id)
    return render(
        request,
        "staff_template/manage_subjectss.html",
        {"subjects": subjects},
    )

def view_schedule(request):
    schedules = Schedules_m.objects.filter(staff_id=request.user.id)
    return render(
        request,
        "staff_template/view_schedule.html",
        {"schedules": schedules},
    )
...
def manage_students(request):
    ...
    return render(
        request,
        "staff_template/manage_students.html",
        {"students": students},
    )
...
def staff_add_result(request):
    ...
    return render(
        ...
        {"subjects": subjects, "courses": courses, "semesters":semesters, "levels":levels},
    )
...
def start_live_classroom(request):
    ...
    return render(
        request,
        "staff_template/start_live_classroom.html",
